...
public class TaxiBooking {
	...
	public static String booking(char pickupPoint,char dropPoint,int pickupTime) throws CloneNotSupportedException
	{
		...
		for(Taxi t : taxiList)
		{
			if(pickupTime>=t.getDropTime() && Math.abs(pickupPoint - (t.getCurrentPoint()))<= min)
			{
				// if Math.abs(..) is equal to min, the taxi with lowest earnings will be put in taxiReady object
				if(Math.abs(pickupPoint - t.getCurrentPoint()) == min)
				{
				...
				{
					taxiReady = t;
					min = Math.abs(pickupPoint - t.getCurrentPoint());
				}
			}
		}
		...
		if(taxiReady != null)
		{
			...
			taxiReady.setCurrentPoint(dropPoint);
